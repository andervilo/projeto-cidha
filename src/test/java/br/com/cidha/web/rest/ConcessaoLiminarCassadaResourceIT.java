...
public class ConcessaoLiminarCassadaResourceIT {
    ...
    public void createConcessaoLiminarCassada() throws Exception {
        ...
        restConcessaoLiminarCassadaMockMvc
            .perform(
                post("/api/concessao-liminar-cassadas")
    ...
    public void getAllConcessaoLiminarCassadas() throws Exception {
        ...
        restConcessaoLiminarCassadaMockMvc
            .perform(get("/api/concessao-liminar-cassadas?sort=id,desc"))
    ...
    public void getConcessaoLiminarCassada() throws Exception {
        ...
        restConcessaoLiminarCassadaMockMvc
            .perform(get("/api/concessao-liminar-cassadas/{id}", concessaoLiminarCassada.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    @Test
    @Transactional
    public void getAllConcessaoLiminarCassadasByProcessosIsEqualToSomething() throws Exception {
        // Initialize the database
        concessaoLiminarCassadaRepository.saveAndFlush(concessaoLiminarCassada);
        Processo processos = ProcessoResourceIT.createEntity(em);
        em.persist(processos);
        em.flush();
        concessaoLiminarCassada.addProcessos(processos);
        concessaoLiminarCassadaRepository.saveAndFlush(concessaoLiminarCassada);
        Long processosId = processos.getId();

        // Get all the concessaoLiminarCassadaList where processos equals to processosId
        defaultConcessaoLiminarCassadaShouldBeFound("processosId.equals=" + processosId);

        // Get all the concessaoLiminarCassadaList where processos equals to processosId + 1
        defaultConcessaoLiminarCassadaShouldNotBeFound("processosId.equals=" + (processosId + 1));
    }
    ...
    private void defaultConcessaoLiminarCassadaShouldBeFound(String filter) throws Exception {
        restConcessaoLiminarCassadaMockMvc
            .perform(get("/api/concessao-liminar-cassadas?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(concessaoLiminarCassada.getId().intValue())))
            .andExpect(jsonPath("$.[*].descricao").value(hasItem(DEFAULT_DESCRICAO)));
        ...
        restConcessaoLiminarCassadaMockMvc
            .perform(get("/api/concessao-liminar-cassadas/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getNonExistingConcessaoLiminarCassada() throws Exception {
        // Get the concessaoLiminarCassada
        restConcessaoLiminarCassadaMockMvc
            .perform(get("/api/concessao-liminar-cassadas/{id}", Long.MAX_VALUE))
    ...
    public void updateConcessaoLiminarCassada() throws Exception {
        ...
        restConcessaoLiminarCassadaMockMvc
            .perform(
                put("/api/concessao-liminar-cassadas")
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    public void deleteConcessaoLiminarCassada() throws Exception {
        ...
        restConcessaoLiminarCassadaMockMvc
            .perform(delete("/api/concessao-liminar-cassadas/{id}", concessaoLiminarCassada.getId()).accept(MediaType.APPLICATION_JSON))
