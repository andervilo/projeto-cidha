...
public class TipoDecisaoResourceIT {
    ...
    public void createTipoDecisao() throws Exception {
        ...
        restTipoDecisaoMockMvc
            .perform(
                post("/api/tipo-decisaos").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(tipoDecisao))
            )
    ...
    public void getAllTipoDecisaos() throws Exception {
        ...
        restTipoDecisaoMockMvc
            .perform(get("/api/tipo-decisaos?sort=id,desc"))
    ...
    public void getTipoDecisao() throws Exception {
        ...
        // Get the tipoDecisao
        restTipoDecisaoMockMvc
            .perform(get("/api/tipo-decisaos/{id}", tipoDecisao.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    @Test
    @Transactional
    public void getAllTipoDecisaosByProcessoIsEqualToSomething() throws Exception {
        // Initialize the database
        tipoDecisaoRepository.saveAndFlush(tipoDecisao);
        Processo processo = ProcessoResourceIT.createEntity(em);
        em.persist(processo);
        em.flush();
        tipoDecisao.addProcesso(processo);
        tipoDecisaoRepository.saveAndFlush(tipoDecisao);
        Long processoId = processo.getId();

        // Get all the tipoDecisaoList where processo equals to processoId
        defaultTipoDecisaoShouldBeFound("processoId.equals=" + processoId);

        // Get all the tipoDecisaoList where processo equals to processoId + 1
        defaultTipoDecisaoShouldNotBeFound("processoId.equals=" + (processoId + 1));
    }
    ...
    private void defaultTipoDecisaoShouldBeFound(String filter) throws Exception {
        restTipoDecisaoMockMvc
            .perform(get("/api/tipo-decisaos?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(tipoDecisao.getId().intValue())))
            .andExpect(jsonPath("$.[*].descricao").value(hasItem(DEFAULT_DESCRICAO)));
        ...
        restTipoDecisaoMockMvc
            .perform(get("/api/tipo-decisaos/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getNonExistingTipoDecisao() throws Exception {
        // Get the tipoDecisao
        restTipoDecisaoMockMvc.perform(get("/api/tipo-decisaos/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    public void updateNonExistingTipoDecisao() throws Exception {
        ...
        restTipoDecisaoMockMvc
            .perform(
                put("/api/tipo-decisaos").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(tipoDecisao))
            )
            .andExpect(status().isBadRequest());
    ...
    public void deleteTipoDecisao() throws Exception {
        ...
        restTipoDecisaoMockMvc
            .perform(delete("/api/tipo-decisaos/{id}", tipoDecisao.getId()).accept(MediaType.APPLICATION_JSON))
